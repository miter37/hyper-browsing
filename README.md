...
> 💬 *"내 링크드인에 들어가서 최신 피드 상위 5개 글 읽고, 주요 트렌드랑 핵심 내용 3줄로 요약해줘."*
- **에이전트 반응**: 보존된 로그인 세션으로 링크드인에 접근하여 전용 러너(`linkedin_runner.mjs`)를 통해 피드를 긁어와 브리핑합니다.

### 예시 2) 복잡한 실시간 시세 / 예매 / 커머스 탐색 요청
> 💬 *"KREAM에서 '나이키 트래비스 스캇 조던 1' 검색해서 270 사이즈 현재 즉시 구매가랑 최근 거래 가격 비교해줘."*
- **에이전트 반응**: 크림 사이트에 접속하여 지문 검증을 통과한 뒤 실시간 시세와 최근 거래 테이블을 구조화된 데이터로 정리해 줍니다.
...
- **에이전트 반응**: 1단계 빠른 요소 파악으로 페이지 구조를 분석해 데이터를 추출한 뒤, `sites/news_ycombinator_com/` 아래에 전용 러너와 지식 문서를 스스로 생성하여 저장합니다.
